...
export default function RealDataSankeyNextPage() {
  ...
  // Top offset reserved for search/year/TopN controls. 表示モードのトークンで決まる
  // （compact-* は2段分の余白を確保。standard-desktop は56）。
  const SEARCH_BOX_RESERVE = Math.round(tokens.searchBoxReservePx * Math.max(1, fontScale));
  ...
  // 列ラベルは fontScaleClampMin で下限クランプ（極小化防止。standard-desktop は 0 = クランプなし）。
  const COLUMN_LABEL_FONT_PX = Math.max(tokens.fontScaleClampMin, scaleFont(tokens.columnLabelFontPx));
        ...
        >
          ...
          {isDockedPanel && !isPanelCollapsed && (
            <div
              ...
              onDoubleClick={() => setSidePanelWidth(tokens.sidePanelWidthPx)}
              style={{
