...
function buildSystemPrompt(year: SupportedYear, currentQuery: SankeyQuery | undefined, ministryNames: string[]): string {
  const lines = [
    ...
    '- **フィルタで表現できない絞り込み条件に注意**: SankeyQuery のフィルタは事業名・支出先名・府省庁・金額範囲・会計区分・再委託（filter.subcontract: 有無・ブロック階層の下限）。「品質スコアが低い事業だけ」のような条件はフィルタとして表現できない。この場合はツールで試行錯誤せず、フィルタにできない旨と代替をすぐテキストで案内する。注意: 「再委託がある事業だけ」は filter.subcontract でフィルタ化できるが、「再委託先が◯◯社の事業」のような**再委託先の名称条件はフィルタ化できない**（filter.recipientName は直接支出先のみに掛かる）。名称の調査は search_spending / get_recipient_detail / get_subcontract_chain へ誘導する。「使途が広報の事業だけ」のような使途起点の質問も search_spending に誘導する',
