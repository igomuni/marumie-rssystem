...
export type {
  PolicyRecommendationTone,
  PolicyQualityInput,
  UnusedTrend,
  PolicyEvaluation,
} from '@/types/policy-evaluation';
import type { PolicyQualityInput, UnusedTrend, PolicyEvaluation, PolicyRecommendationTone } from '@/types/policy-evaluation';
 ...
 *
 * 同点は中間順位（midrank）で返す。AI採点は 0-10 を 0-100 に伸ばした離散値のため
 * 同点が大量に出る。下限位置をそのまま使うと、同点の塊が丸ごと最下位の帯に落ち、
 * 全件同点なら全事業が percentile 0＝最下位帯として推奨判断されてしまう。
 */
function percentileRank(sorted: number[], value: number): number {
  if (sorted.length <= 1) return 100;
  const lowerBound = binarySearch(sorted, value, false);
  const upperBound = binarySearch(sorted, value, true);
  // 同点が無ければ lowerBound === upperBound - 1 で従来と同じ値になる
  const midrank = (lowerBound + upperBound - 1) / 2;
  return (midrank / (sorted.length - 1)) * 100;
}

/** upper=false なら value 以上が現れる最初の位置、true なら value 超が現れる最初の位置 */
function binarySearch(sorted: number[], value: number, upper: boolean): number {
  let lo = 0;
  let hi = sorted.length;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (upper ? sorted[mid] <= value : sorted[mid] < value) lo = mid + 1;
    else hi = mid;
  }
  return lo;
}
