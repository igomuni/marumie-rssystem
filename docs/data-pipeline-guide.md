...
| `npm run generate-recipient-index` | `scripts/generate-recipient-index.ts 2024` | subcontracts-2024.json（パイプライン後段。generate-subcontracts の後に実行） |
| `npm run generate-recipient-index-2025` | `scripts/generate-recipient-index.ts 2025` | 同上（2025年度） |
...
補正予算の `amount`（改予算額）は突き合わせに使えない。事項別内訳には補正対象の
事項しか載らないため、全科目の改予算額の合計にはならないため。
