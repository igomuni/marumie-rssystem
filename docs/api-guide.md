...
過去のレポートが人力で見つけた「発見の型」を6指標として全事業を機械スキャンし、観測可能な「注目シグナル」を列挙する（WP4-1）。**「異常」「無駄」の判定ではない**。

**設計原則（変更時も守ること）**:

- **語彙の規律**: 実装・レスポンス・UI文言のいずれでも「無駄」「不正」「異常」と断定しない。計算するのは観測可能な事実（比率・順位・差分）だけで、解釈は利用者に委ねる
- **重み付き合成スコアを作らない**: 6指標を1つの総合スコアに畳み込むと根拠が追えなくなる。指標別の上位リスト + 複数該当（`multiSignal`）の提示に留める
- **母集団に下限を設ける**: 少額事業は比率指標が過敏に振れるため、比率系の指標には支出額の下限を課す
...
| `view` | 任意 | `transfer` で特別会計の財源内訳を返す。省略時は全体フロー |

**データソース**: `public/data/mof-budget-overview-{year}.json.gz`（年度ごとに1ファイル、プロセス内キャッシュ）。
収録は2017〜2026年度で、対象は**当初予算のみ**。

**レスポンス**: `MOFBudgetOverviewData`（`view=transfer` のときは `MOFTransferDetailData`）。
`metadata.availableYears` は API が応答時に付与する。

> **合算に注意**: `metadata.grossTotal` は会計区分をまたいだ単純合計で、会計間の繰入が
> 二重計上されている。控除後は `metadata.netTotal`。捕捉ロジックは
> `docs/mof-budget-data-guide.md` 6節を参照。

---

## GET /api/mof-hierarchy

事項別内訳を「予算合計 → 所管 → 組織/特会 → 勘定/業務 → 項 → 事項」の6列に畳んだ
階層 Sankey を返す。データ源は `/api/mof-jikou` と同じ生成物で、専用の生成物は持たない。
...
|------|------|------|
| `year` | 任意 | 会計年度（西暦）。省略時は収録済みの最新年度。対象外の年度は400 |
| `budgetType` | 任意 | 予算種別。その年度に無ければ当初予算に落とす |
| `topMinistry` | 任意 | 所管の表示数（**列全体**の上限、最大300）。既定は無制限（実測25件で収まる） |
| `topOrganization` | 任意 | 組織/特会の表示数（同上。既定40） |
| `topSubAccount` | 任意 | 勘定/業務の表示数（同上。既定40） |
| `topSection` | 任意 | 項の表示数（同上。既定40） |
| `topItem` | 任意 | 事項の表示数（同上。既定40） |
| `offsetMinistry` | 任意 | 所管の表示開始位置（0始まり）。範囲外は末尾に丸める |
| `offsetOrganization` | 任意 | 組織/特会の表示開始位置（同上） |
| `offsetSubAccount` | 任意 | 勘定/業務の表示開始位置（同上） |
| `offsetSection` | 任意 | 項の表示開始位置（同上） |
| `offsetItem` | 任意 | 事項の表示開始位置（同上） |
| `filterMinistry` | 任意（複数可） | 所管名で絞る（政府関係機関は「政府関係機関」等の会計区分ラベル） |
| `filterAccount` | 任意（複数可） | 会計区分で絞る（`general`/`special`/`agency`） |
| `filterSection` | 任意 | 項名で絞る（部分一致） |
| `filterSectionRegex` | 任意 | `1` で `filterSection` を正規表現として解釈する |
| `filterItem` | 任意 | 事項名で絞る（部分一致） |
| `filterItemRegex` | 任意 | `1` で `filterItem` を正規表現として解釈する |
| `filterMinAmount` | 任意 | 事項の金額（円）の下限 |
| `filterMaxAmount` | 任意 | 事項の金額（円）の上限 |

**エラー**: 生成物が1年度も無いとき、またはその年度に予算種別が1つも無いときは **503**。
対象外の年度を指定したときは 400（`availableYears` を添えて返す）。

**レスポンス**: `MOFHierarchyData`（型定義: `types/mof-hierarchy.ts`）。
`sankey`（図の描画用・TopNで絞る）と `browse`（サイドパネル用・絞らず全件）の
2系統を持つ。

> **値の無い列は素通りする**。一般会計と、勘定を持たない特別会計は勘定列にノードを持たない
> （どの会計が勘定を持つかは年度による）。
> 図では場所だけ確保する透明な通過ノード（`details.passThrough`）が入る。
>
> **TopN は列全体の表示件数の上限**で、親ノードごとの件数ではない（`/sankey-svg` と同じ）。
> 親ごとに N 件ずつ残すと親の数だけ掛け算で増え、列の総数を抑えられないため。
> 順位付けは「親が残っている枝」の中で行うので、上流を絞ると下流の候補も減る
> （所管→組織→勘定の3列）。
>
> ただし**項・事項は例外**で、親の絞り込みと無関係に全件が候補になる
> （`/sankey-svg` の「事業→支出先」が所属所管の TopN と無関係に `allEdges` から
> グローバルにランキングするのと同じ）。組織が TopN で集約されていても、その配下の
> 項が単独で大きければ項の窓には個別に残る——項が TopN で集約されていても、その配下の
> 事項が単独で大きければ事項の窓には個別に残る。いずれも、帯は直近の実祖先の
> 集約ノードから直接伸びる。項・事項だけをこの扱いにしているのは、画面の表示位置
> （オフセット）対象セレクトがこの2列に絞られているため（所管・組織・勘定は候補件数が
> 少なく、位置をずらす操作自体があまり意味を持たない）。API 自体は
> `offsetMinistry`・`offsetOrganization`・`offsetSubAccount` も引き続き
> サポートしており、`buildMOFHierarchySankey` はこれらにもオフセットを適用する
> ——絞っているのは画面の UI コントロールだけで、API のサポート範囲ではない。
>
> **項自身のTopN選抜も、事項オフセットで見えている額に連動して毎回再ランキングされる**
> （`/sankey-svg` が事業のTopN選抜を「現在窓に入っている支出先からの流入額」で
> 毎回再計算するのと同じ）。事項オフセットを進めて項の配下の事項が窓の手前に
> 隠れると、その項の「見えている額」が真の合計より小さくなり、他の項に順位で
> 抜かれて集約側に回ることがある。順位が入れ替わっても`1,066項`のような
> **件数の数字自体は変わらない**（項自身のTopN・オフセットが動かない限り、
> 全項数からTopNを引いた値は一定）。変わるのは集約に**何が**入っているか
> （`details.aggregatedTop`）。組織・勘定・所管はこの再ランキングの対象外
> （`/sankey-svg` の所管と同じ）。
>
> **項は必ず1件以上の事項を持つ**（事項ゼロの項は仕組み上作られない）。
> 事項オフセットで、ある項の配下の事項が**全件**窓の手前に隠れると、
> その項自身も隠す（kept にも集約にも現れない）。表示すべき中身が
> 無くなった項を残すと、0円の箱や中身のない集約エントリが出てしまうため
> （`/sankey-svg` が可視流入ゼロの事業をTopN候補からそもそも除くのと同じ）。
> `columnCounts.section` はこの分だけ減る（実測: 事項オフセットを大きく
> 進めると854→88のように大きく減ることがある）。
>
> **項オフセットで項自身が窓の手前へ明示的に隠れた場合、配下の事項は
> kept でも道連れで隠す**（事項は項より先に決まるため、項自身の
> TopN・オフセット無しなら配下の事項は先に kept になっていることがある）。
> 項がTopN選抜で単に集約（tail）に回っただけなら、配下の事項が単体で
> 事項の窓に残っていれば集約ノードから帯を伸ばして個別に出続ける
> （上記の再ランキングの話）。両者の違いは「項自身がユーザーの操作で
> 明示的に押し出されたか」——前者は関係する事項ごと消え、後者は
> 事項単体では生き残りうる。
>
> **項オフセットの道連れで事項の窓（TopN）が欠けたら、次点を繰り上げて埋め戻す**。
> 項が明示的に隠れて配下の事項も一緒に消えると、事項の表示件数が設定した
> TopNより少なくなることがある。「上位N件を表示する」という約束を保つため、
> 欠けた分だけ集約に回っていた次点の事項を個別表示に繰り上げる
> （実測: 既定のTopN=40で項オフセットをかけても事項の表示件数は常に40のまま）。
>
> **項・事項の消滅で配下が空になった祖先（勘定・組織・所管）も同じ扱い**。
> 勘定→組織→所管の順（下から上）に「配下（通過ノードは透過）に実体が
> まったく残っていないか」を見て、空になっていれば道連れで隠す。
> 隠れて欠けた分は、その列自身のTopNまで次点を繰り上げて埋め戻す
> （所管・組織・勘定・事項のすべてで同じ埋め戻しを行う。項は既に
> 独立した埋め戻しを持つため対象外）。ただし埋め戻す候補自体が
> 尽きれば、その分は表示件数がTopNを下回ることもある
> （実測: 事項オフセットを極端に進めると組織の表示件数がTopN=40を
> 下回ることがあるが、これは埋め戻す候補自体が尽きたため正常）。
> **窓より下位に溢れた分は列ごとに1つの集約ノードにまとまる**（`details.aggregated`）。
> 名前は `41組織` `1,672事項` のように件数と単位で、「その他」とは書かない。
> `details.aggregatedCount` に件数、`details.aggregatedTop` に金額上位の内訳が入る。
> 窓より上位（オフセットで手前に外れた分）はここには入らない（下記）。
> 列内で常に「表示中の実ノード + この集約」の合計が `metadata.total` と一致する。
>
> **表示開始位置（offset）で窓をずらせる**。TopN だけだと上位しか見られず、
> 窓から外れた順位は集約の中に消えたまま辿れないため。
> 窓より**下位**（末尾）は集約ノードに入る。窓より**上位**（手前）は
> `/sankey-svg` と同じく**どこにも表示せず、集約にも含めない**——祖先
> （直近の親から予算合計まで）の金額からも実際に差し引く。
> つまりオフセットを進めると、その分だけ `metadata.total`・所管・組織などの
> 表示金額そのものが縮む（既定表示＝オフセット無しでは今までどおり全量のまま）。
> 丸めた後の値は `metadata.offset`、列ごとの候補件数は `metadata.columnCounts`
> に入る（画面が「◯〜◯ / 全◯件」を出すのに使う。分母はオフセットで変わらない）。
>
> **根は会計区分をまたいだ単純合計**で、会計間の繰入が二重計上されている。
> 会計区分ごとの内訳は `accounts` に入る（オフセット無しの既定表示での値）。
>
> **収録されていない会計区分は現れない**。決算の事項別内訳は一般会計にしかない。
>
> **絞り込みは階層を組む前（事項別内訳の生データ）に効く**（`app/lib/mof-hierarchy-filter.ts`）。
> `/sankey-svg` のフィルタパネルと同じ発想の条件セットを列に合わせて置き換えている
> （事業名→事項名、支出先名→項名、省庁→所管はそのまま）。所管や会計を絞ると、
> 該当が無くなった組織・勘定はそもそも現れない（`sankey` にも `browse` にも出ない）。
> 不正な正規表現は何も除外しない側に倒す（フィルタの不備で図が全滅するより実害が小さい）。
>
> **`browse` は集約せず、実ノードのみを全件返す**（`/sankey-svg` が常にフルデータを
> パネル用に保持しているのと同じ考え方）。`sankey` が TopN で絞って集約ノードに
> 畳んでいる分も、`browse` では個々の実ノードとして残る。集約された所管の下でも
> 実ノードとして辿れるので、`details.aggregated` は常に付かない。件数は`sankey`より
> 大きく、令和8年度当初で事項は1,712件（`sankey` 側は既定40件+集約1件）。
