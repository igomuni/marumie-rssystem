...
## 追記: 事項（葉）の候補件数が93件止まりだった件（同日・別リクエストで対応）

上記の実装後、「事項のTopNオフセットが93までしか動かない」との指摘。原因は既存の
「上流を絞ると下流の候補も減る」設計（4列: 所管→組織→勘定→項）が、項→事項にも
そのまま効いていたこと（既定の項TopN=40 の配下にある事項だけが候補になり、
全1,712件中93件しか事項側で辿れなかった）。

`/sankey-svg` の実装を確認すると、所管→事業は事業側を所属所管の TopN に従属させて
候補を絞るが、**事業→支出先（葉）だけは例外**で、`allEdges` から所属事業と無関係に
グローバルにランキングしている（`app/lib/sankey-svg-filter.ts:136-143`）。ユーザーの
希望（「/sankey-svgと同じようにしたい」）に基づき、mof-hierarchy も**葉（事項）だけ**
同じ例外にした。中間列（組織・勘定・項）は今までどおり上流の kept 状態に従属させる
（/sankey-svg のミニストリ→事業と同じ）。

実装（`app/lib/mof-hierarchy-sankey.ts`）:
- 列ごとの候補判定で、`column === 'item'` のときだけ `kept.has(parentId)` を要求せず、
  代わりに `!hidden.has(node.id)`（窓の手前で隠されたセクションの配下でないこと）だけを見る
- 帯の起点解決を `resolveSource(parentId)` に一本化。親が kept ならそこから、
  kept でなければ親の列の集約ノード（`__others__section` 等）から繋ぐ。
  これにより、項が集約された配下からでも事項単体で TopN の窓に残れば、
  帯は項の集約ノードから直接伸びる
- `columnCounts.item` は全事項件数（既定設定で1,712件）になる

ユニットテスト4本・e2eテスト1本を追加（うち3本が旧実装に対して赤であることを確認）。
