...
> 注 1: 「単独設定状態」は **`filterMinistryNames` の状態のみ**で判定し、`selectedNodeId` や `filterActive` には依存しない。これにより以下を保証する：
> - 「省庁A 選択 → 空白クリックで非選択 → 省庁A 再クリック」でフィルタ解除
> - 検索/フィルタモードトグル（`filterActive`）が OFF でも省庁ノードクリックでフィルタ解除可能
>
> 注 2: `filterMinistryNames` は `filterActive` と独立に visualization に作用するため（`hasMinistry = filterMinistryNames.length > 0`）、`filterActive` を切り替える必要はない。
...
| 8 | 検索/フィルタモード OFF + 省庁A クリック → 省庁A クリック | 1 で snap 保存・filter=[A] / 2 で snap 復元（filterActive を変更しないため OFF のままでも動作） |
...
- **`filterActive` には触れない**: `filterMinistryNames` は `filterActive` と独立に作用する（[page.tsx:844](../../app/sankey-svg/page.tsx#L844): `hasMinistry = filterMinistryNames.length > 0`）ため、トグルする必要がない。検索/フィルタモードを OFF にしている状態でも省庁ノードクリックは正常に機能する。
