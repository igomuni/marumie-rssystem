...
**「すべて選択/解除」（マスター）がONの間、個別チェックボックスは表示上すべてOFFにする
（2026-09-15変更）。** 以前はマスターON（`selected===null`）のとき個別も全部チェック済み
に見せていたが、「1つの`すべて選択/解除`で全部含むことを表しているのに、個別も全部
チェック済みに見えるのは冗長・紛らわしい」との指摘を受けた。新しい状態遷移：
- マスターON（`selected===null`）：個別は全部OFF表示
- 個別を1つでもONにする → その1件だけの明示選択（`selected=[v]`）に切り替わり、
  マスターは自動でOFFになる（`selected!==null`なので）
- マスターOFFの間の個別チェック・解除は通常の配列操作（`selected`への追加・削除）
- 全解除（0件・グラフから全除外）にするには、まずマスターON（`selected===null`）の
  状態にしてからマスターをもう一度押す（`selected=[]`になる）——`checked ? []
  : null`という単純なトグルではなく、マスター自身のON/OFFで意味が変わる非対称な
  トグルになった
...
タブ：「目」（接続先事業・状態・金額。目ごとに予算種別バッジ`当初`/`補正N`を表示。
補正予算の目は`amount`ではなく`difference`＝差額）、「RS事業」（この項から接続する
事業の合計額）。

「目」タブの未接続（status: `'unconnected'`）行は、以前は「RS未接続」という
メタ文言を表示していたが、事実の割に目立ちすぎる／誤解を招くとの指摘を受けて
何も表示しないことにした（良い代替案が出るまでの暫定、2026-09-14）。
「超過・要確認」（status: `'excess'`）はそのまま残す。
