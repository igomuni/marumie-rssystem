...
- **目**：`itemKey`（MOFの目レコードの識別子。予算種別を含むため当初と補正は
  ...
  ラベルは`RS × N`にしてほしい」との指摘で移動、2026-09-15）。バッジの見た目は
  `OutlineBadge`（白背景＋色付き枠線＋黒系文字。色は`#78909c`）——当初は塗りつぶし
  の`Badge`だったが、隣に並ぶ`BudgetTypeBadge`（同じくoutline系）と見た目の
  系統が揃わず「他と違和感がある」との指摘を受け、outline系に統一した
  （2026-09-15）。未接続（status: `'unconnected'`）行はメタ文言なし（「RS未接続」は事実の割に
