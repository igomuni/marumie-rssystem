...
- **サマリー**：この項の接続状況の集計。RS接続額／未接続額／超過額（要確認）
  （`itemEdges`をstatus別に合算）、目数（当初・補正の内訳付き）、接続RS事業数。
...
- **目**：`itemKey`（MOFの目レコードの識別子。予算種別を含むため当初と補正は
  ...
  予算種別バッジ（`当初`/`補正N`）に加えて、接続するRS事業の件数を
  「RS事業N件」バッジ（グレー）で示す。未接続（status: `'unconnected'`）行は
  メタ文言なし（「RS未接続」は事実の割に目立ちすぎる／誤解を招くとの指摘を受けて
  廃止、良い代替案が出るまでの暫定、2026-09-14）。超過（status: `'excess'`）が
  1件でもあれば「超過・要確認」を出す
- **RS事業**：この項から接続する事業ごとの合計額（`itemEdges`を`target`で集計）。
  2行目に予算種別ごとの接続件数をバッジで表示（`BudgetTypeBadge`の`count`引数、
  2026-09-15追加）。**バッジのラベルと件数の間には`×`区切りが必須**——補正予算の
  ラベルには既に号数の数字が入っており（例:「補正1」）、区切り無しで件数を続けると
  「補正1」＋「1件」が「補正11件」に読めてしまう（実際は補正1号への1件の接続なのに
