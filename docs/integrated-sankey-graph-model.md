...
タブは4つ、左から **予算サマリ→MOF項→予算執行→目** の順（2026-09-14変更。
...
- **MOF項**：この事業に接続するMOF項の名前と金額の一覧（`itemEdges` を
  `source`＝MOF項IDで集計した`bySection`）。以前は「目」タブ（RS自身の予算内訳を
  接続済み/未接続の二値バッジで示すのみ）がこの位置にあったが、実際に紐づいた
  MOF項の名前・金額をそのまま見せた方がつながりを直感的に表現できるという
  指摘を受け、予算サマリタブの中に埋まっていた同内容を独立タブへ格上げした
  （「目」タブ自体は、RS側の未接続項目の可視性という別の価値があるため残す）
...
- **目**：`budgetItems`（`budgetBreakdown` を当年度・当初予算＋補正予算（決算等は
  除く）に絞りMOF紐づけ状況を付与したもの、MOF接続済み/未接続バッジ付き）。
  当初予算のみに絞ると、紐づけが補正予算経由でしか無いRS側の歳出予算項目が
  一覧から丸ごと消え、実際は接続しているのに確認できなくなる不具合があった
  （`isRsPrimaryBudgetType`。`budgetItemMatchesLink` 側の判定も同様に広げてある）
