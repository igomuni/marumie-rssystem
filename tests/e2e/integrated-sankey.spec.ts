...
test('事業サイドパネルに支出先・ブロック・ブロックのつながりタブが出て、再委託構造（既存データ）を確認できる', async ({ page, request }) => {
  ...
  await expect(detail.getByRole('button', { name: 'ブロックのつながり', exact: false })).toBeVisible();
