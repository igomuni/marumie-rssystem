...
1. **変更内容の把握**: `git diff` で変更されたファイルの差分を取得し、変更内容を把握する。必要に応じて関連ファイルも読み込む。変更の背景となった設計ドキュメントは [docs/tasks/INDEX.md](docs/tasks/INDEX.md) から特定する。
...
3. **レビュー実施**: 気づいた問題は重要度で切らずに全件挙げ、そのうえで「要修正 / 改善提案」に振り分ける。以下の観点でレビューを行う：
   - **アーキテクチャ**: レイヤー設計との整合性
