...
/** クラスタ定義（左→右の順序） */
const CLUSTER_TYPES = ['total', 'ministry', 'project-budget', 'project-spending', 'spending-block', 'recipient'] as const;
...
function main() {
  ...
  // block → recipient エッジからblockのministry属性を経由して帰属を決定
  // フォールバック: project-spending → recipient 直接エッジも考慮
  console.log('\n[2/5] 支出先の府省庁帰属計算');
  const recipientMinistry = new Map<string, string>();
  const recipientMinistryTotals = new Map<string, Map<string, number>>();

  for (const edge of graph.edges) {
    if (!edge.target.startsWith('recipient-')) continue;
    const sourceNode = nodeMap.get(edge.source);
    if (!sourceNode?.ministry) continue;
    // block → recipient or project-spending → recipient (fallback)
    if (!edge.source.startsWith('block-') && !edge.source.startsWith('project-spending-')) continue;
  ...
  for (let ci = 0; ci < CLUSTER_TYPES.length; ci++) {
    ...
    if (clusterType === 'total') {
      ...
      for (const bn of blockNodes) {
        if (bn.projectId === undefined) continue;
        const list = blocksByProject.get(bn.projectId) ?? [];
        list.push(bn);
        blocksByProject.set(bn.projectId, list);
      }

      for (const [projectId, blocks] of blocksByProject) {
        // 親事業のレイアウト位置を取得
        ...
        const items: TreemapItem[] = blocks.map(n => ({
          key: n.id,
          value: n.amount,
          data: n,
        }));

        const results = squarifiedTreemap(items, bandRect);
        for (const res of results) {
          if (!res.data) continue;
          const r = applyGap(res.rect, NODE_GAP);
          const ln: LayoutNode = {
  ...
  for (const e of graph.edges) {
    if (e.value > maxEdgeValue) maxEdgeValue = e.value;
  }
