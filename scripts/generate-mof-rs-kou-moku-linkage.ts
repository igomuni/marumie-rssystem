 ...
 * RS の `2-2_予算・執行_予算種別・歳出予算項目` は 所管/組織・勘定/項/目 を MOF の
 * 科目別内訳（mof-kou-moku-{年度}.json）と同じ語彙で持つため、名前照合や構造キーでの
 * 絞り込みは不要で、完全一致キーで直接突き合わせられる
 * （実測: 一般会計・当初予算でRS行の92.7%・金額の97.9%が一致。
 ...
 *
 * 入力:
 *   public/data/mof-kou-moku-{予算年度}.json  … MOF目（一般会計・当初予算のみ使用）
...
function main() {
  ...
  for (const row of budgetItemRows) {
    ...
    if (existing) {
      ...
      linkMap.set(pairKey, {
        ...
        mofOrganization: kouMoku.organization,
  ...
  const output: MofRsKouMokuLinkageData = {
    metadata: {
      ...
      notes: [
        '所管×組織・勘定×項×目の完全一致キーで直接突き合わせている（名前照合・語幹一致は使わない）',
        'RS の 2-2 CSV は所管/組織・勘定/項/目をMOFの科目別内訳と同じ語彙で持つため、この紐づけに誤検出は原理上ない',
