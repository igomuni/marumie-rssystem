...
import type { RsSpendingBlockRecord, RsRecipientRecord, RsContractRecord, SourceRef } from '../types';
...
function parseBoolOrNull(raw: string | undefined): boolean | null {
  const s = (raw ?? '').trim();
  if (!s) return null;
  if (['TRUE', '1', 'YES', 'Y', '○', '有', 'あり'].includes(s.toUpperCase()) || ['○', '有', 'あり'].includes(s)) return true;
  if (['FALSE', '0', 'NO', 'N', '×', '無', 'なし'].includes(s.toUpperCase()) || ['×', '無', 'なし'].includes(s)) return false;
  return null;
}

export function normalizeSpending(
  rawRoot: string, zipPath: string, entry: string, rows: Record<string, string>[], year: number
): { blocks: RsSpendingBlockRecord[]; recipients: RsRecipientRecord[]; contracts: RsContractRecord[] } {
  ...
  rows.forEach((row, i) => {
    ...
    if (bid && (rname || corp || loc || ctype || other || (row['支出先の合計支出額'] ?? '').trim())) {
      ...
      if (!recipients.has(rkey)) {
        ...
        recipients.set(rkey, {
          ...
          location: loc,
          corporateType: ctype,
          otherRecipient: parseBoolOrNull(other),
          totalAmountValuesYen: [],
          evidenceRowIds: [],
          sources: [],
        });
    ...
    if (bid && CONTRACT_COLUMNS.some(c => (row[c] ?? '').trim())) {
      ...
      contracts.push({
        ...
        otherContract: parseBoolOrNull(row['その他の契約']),
        sourceRowId: rowId,
        source,
      });
    }
  ...
  return { blocks: [...blocks.values()], recipients: [...recipients.values()], contracts };
}
