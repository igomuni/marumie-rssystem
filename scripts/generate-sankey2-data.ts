...
interface Sankey2Node {
  ...
  amount: number;       // 1円単位
  /** project-budget/project-spending/spending-block のみ: 予算事業ID */
  projectId?: number;
  /** ministry / project / spending-block のみ: 府省庁名 */
  ministry?: string;
...
function main() {
  ...
  // 5d. 支出先ノード（エッジはブロック経由で生成するため、ここではノードのみ）
  ...
  console.log('\n[3.5/4] ブロックノード・エッジ構築（5-2 CSV + 5-1 CSV）');
  ...
  for (const row of blockRows) {
    ...
    // ブロック名を記録
    if (target && targetName) blockNameMap.set(`${pid}:${target}`, targetName);
    if (source && sourceName) blockNameMap.set(`${pid}:${source}`, sourceName);
  ...
  // 5g. spending-block ノード生成 + エッジ生成
  // 全ブロック（pid:block）をノード化し、3種のエッジを生成:
  //   - project-spending → block（直接支出ブロック）
  //   - block → block（再委託接続）
  //   - block → recipient（ブロック内支出先）
  ...
  // 全ユニークブロックを収集してノード化
  const allBlocks = new Set<string>(); // "pid:block"
  for (const key of blockSpending.keys()) allBlocks.add(key);
  for (const key of blockNameMap.keys()) allBlocks.add(key);

  let blockNodeCount = 0;
  const blockNodeIds = new Map<string, string>(); // "pid:block" → nodeId

  for (const key of allBlocks) {
    const [pidStr, block] = key.split(':');
    const pid = parseInt(pidStr, 10);
    if (!orgMap.has(pid)) continue;
    const org = orgMap.get(pid)!;
    const blockName = blockNameMap.get(key) || block;
    const recipients = blockSpending.get(key) ?? [];
    const amount = recipients.reduce((sum, r) => sum + r.amount, 0);

    const nodeId = `block-${pid}-${block}`;
    blockNodeIds.set(key, nodeId);
  ...
  console.log(`  ブロックノード: ${blockNodeCount.toLocaleString()} 件`);

  // project-spending → block エッジ（直接支出ブロック）
  let psToBlockEdges = 0;
  for (const [pid, directBlocks] of directBlocksByProject) {
    for (const [block] of directBlocks) {
      const blockNodeId = blockNodeIds.get(`${pid}:${block}`);
      if (!blockNodeId) continue;
      const recipients = blockSpending.get(`${pid}:${block}`) ?? [];
      const amount = recipients.reduce((sum, r) => sum + r.amount, 0);
  ...
  console.log(`  project-spending → block エッジ: ${psToBlockEdges.toLocaleString()} 件`);

  // block → block エッジ（再委託接続）
  let blockToBlockEdges = 0;
  for (const [pid, connections] of blockChainByProject) {
    for (const conn of connections) {
      const sourceId = blockNodeIds.get(`${pid}:${conn.source}`);
      const targetId = blockNodeIds.get(`${pid}:${conn.target}`);
      if (!sourceId || !targetId) continue;
      if (conn.amount <= 0) continue;
      edges.push({
        source: sourceId,
        target: targetId,
        value: conn.amount,
  ...
  console.log(`  block → block エッジ: ${blockToBlockEdges.toLocaleString()} 件`);

  // block → recipient エッジ（ブロック内支出先）
  let blockToRecipEdges = 0;
  for (const [key, recipients] of blockSpending) {
    const blockNodeId = blockNodeIds.get(key);
    if (!blockNodeId) continue;
    for (const r of recipients) {
      const recipientNode = recipientNodeIndex.get(r.name);
  ...
  console.log(`  block → recipient エッジ: ${blockToRecipEdges.toLocaleString()} 件`);

  // ブロックデータがない事業のフォールバック: project-spending → recipient 直接エッジ
  let fallbackEdges = 0;
  const pidsWithBlocks = new Set<number>();
  for (const key of allBlocks) {
    const pid = parseInt(key.split(':')[0], 10);
    pidsWithBlocks.add(pid);
  }
  for (const [, recipient] of recipientMap) {
    for (const [pid, amount] of recipient.projectAmounts) {
      if (pidsWithBlocks.has(pid)) continue; // ブロック経由のエッジがある
      edges.push({
        source: `project-spending-${pid}`,
        target: `recipient-${recipient.name}`,
        value: amount,
      });
      fallbackEdges++;
    }
  }
  console.log(`  フォールバック直接エッジ: ${fallbackEdges.toLocaleString()} 件（ブロックなし事業）`);
  ...
  for (const [pid] of orgMap) {
    ...
    for (const [bsKey, recipients] of blockSpending) {
      ...
      if (!directBlocks.has(block)) {
        for (const r of recipients) indirectRecipientNames.add(r.name);
      }
    }
  }
